...
void DES_Pattern::PermChoice1() //Permutation Choice-1
{
	int k = 57, i;
	for(i = 0; i<28; i++) {
		pc1[i] = key[k - 1];
...
void DES_Pattern::Expansion() //Expansion Function applied on `right' half
{
	int exp[8][6], i, j, k;
	for(i = 0; i<8; i++) {
		for(j = 0; j<6; j++) {
...
void DES_Pattern::xor_oneE(int round) //for Encrypt
{
	int i;
	for(i = 0; i<48; i++)
		xor1[i] = expansion[i] ^ keyi[round - 1][i];
}
void DES_Pattern::xor_oneD(int round) //for Decrypt
{
	int i;
	for(i = 0; i<48; i++)
...
char * DES_Pattern::encrypt(const char *Text1) {
...
char * DES_Pattern::decrypt(const char *Text1) {
	int i, a1, j, nB, m, iB, k, K, B[8], n, t, d, round;
	char *Text = new char[1000];
	unsigned char ch;
	strcpy(Text, Text1);
